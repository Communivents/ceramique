...
async function build(arch: keyof typeof builds) {
	...
	await $`bun build --compile --sourcemap --minify-syntax --minify-whitespace --target=${build.target} ./src/index.ts --outfile ./dist/${build.name}`;
}

async function buildDefault() {
	plogger.info('Compiling app...');
	await $`bun build --compile --sourcemap --minify-syntax --minify-whitespace ./src/index.ts --outfile ./dist/${binaryName}`;
}
